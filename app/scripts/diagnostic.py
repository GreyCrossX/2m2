...
def get_first_last(stream: str) -> Tuple[Optional[Tuple[str, Dict]], Optional[Tuple[str, Dict]]]:
    first = redis_client.xrange(stream, count=1)
    last  = redis_client.xrevrange(stream, count=1)
    f = first[0] if first else None
...
def check_ethusdt():
    ...
    if market_len > 0:
        f, l = get_first_last(MARKET)
        if f and l:
            f_ts, f_id_ms, _ = ts_from_entry(f)
            l_ts, l_id_ms, _ = ts_from_entry(l)
            ...
            print(f"   First message ID: {f[0]}  ts={f_ts}  (id_ms={f_id_ms})")
            print(f"   Last  message ID: {l[0]}  ts={l_ts}  (id_ms={l_id_ms})")
    ...
    if market_len > 0 and ind_len > 0:
        ...
        if m_ms is not None and i_ms is not None:
            ...
            print(f"   Market last:    id={m_last[0]} ts={m_last_ts} (id_ms={m_last_id_ms})")
            print(f"   Indicator last: id={i_last[0]} ts={i_last_ts} (id_ms={i_last_id_ms})")
            ...
            if gap_ms > 0:
                ...
                start_id = m_last[0]  # we’ll actually start from indicator last id to find "after"
                ...
                unprocessed = redis_client.xrange(MARKET, min=after_id, max='+', count=5)
                if unprocessed:
                    ...
                    print("   ⚠️  No subsequent market messages found after indicator’s last ID.")
        else:
    ...
    if ind_len == 0:
        ...
        print(f"   ❌ {market_len - ind_len} messages behind - processor likely stopped")
