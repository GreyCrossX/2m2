    ...
    try:
        while True:
            ...
            for symbol in symbols:
                ...
                processing_lag = (
                    market_ts - ind_ts if market_ts > 0 and ind_ts > 0 else 0
                )
                ...
                print("-" * 90)
                print(
                    f"  Market:      {market_len:6d} msgs | Last: {market_id:20s} | Age: {format_time_ago(market_age):>8s}"
                )
                print(
                    f"  Indicators:  {ind_len:6d} msgs | Last: {ind_id:20s} | Lag: {format_time_ago(processing_lag):>8s}"
                )
                print(
                    f"  Signals:     {sig_len:6d} msgs | Last: {sig_id:20s} | Lag: {format_time_ago(signal_lag):>8s}"
                )
