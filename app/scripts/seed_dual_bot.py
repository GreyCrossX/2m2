...
SYMBOLS = [
    s.strip().upper()
    for s in os.getenv("SEED_SYMBOLS", "BTCUSDT,ETHUSDT").split(",")
    if s.strip()
]
...
def ensure_user_and_cred(session: "Session", user_id: str, cred_id: str, env: str):
    ...
    c = session.execute(
        select(ApiCredential).where(ApiCredential.id == cred_id)
    ).scalar_one_or_none()
    if not c:
    ...
    if getattr(c, "env", None) and str(c.env) != env:
        print(
            f"[WARN] Credential env is '{c.env}', but target env is '{env}'. Proceeding…"
        )
    return u, c


def upsert_bot_real(
    session: "Session", *, user_id: str, cred_id: str, symbol: str
) -> "Bot":
    b = session.execute(
...
if __name__ == "__main__":
    ...
    print(
        f"[setup] Risk={_to_str(RISK_PCT)} leverage={_to_str(LEVERAGE)} tp_ratio={_to_str(TP_RATIO)}"
    )
    ...
    if USE_DB:
        ...
        try:
            with Session(engine) as s:
                ...
                for sym in SYMBOLS:
                    b = upsert_bot_real(
                        s, user_id=str(u.id), cred_id=str(c.id), symbol=sym
                    )
        ...
        for i, sym in enumerate(SYMBOLS, start=1):
            bot_ids_by_symbol[sym] = os.getenv(
                f"SEED_BOT_ID_{sym}", f"bot-{sym.lower()}-dev"
            )
    ...
    for sym in SYMBOLS:
        ...
        print(
            f"[Redis] BotConfig readback for {sym}:",
            read_bot_config(bot_ids_by_symbol[sym]),
        )
    ...
    if EMIT_FAKE:
        for sym in SYMBOLS:
            msg_id = emit_fake_arm(
                sym=sym, side="long", trigger="125000.0", stop="124800.0"
            )
    ...
    print(
        "Done. If pollers/workers are up, signals will now dispatch to BOTH bots when calc emits."
    )
