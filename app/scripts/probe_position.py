...
def main():
    ...
    ap.add_argument(
        "--user", required=True, help="User id passed to facade (for creds lookup)."
    )
    ap.add_argument(
    ...
    missing_envs = [
        e
        for e in ("BINANCE_USDSF_API_KEY", "BINANCE_USDSF_API_SECRET")
        if not os.getenv(e)
    ]
    if missing_envs:
        LOG.warning(
            "Missing envs: %s (ok if you use set_creds_lookup)", ", ".join(missing_envs)
        )
    ...
    assert_ok(
        ...
        "selftest failed (exchange_information/position_mode)",
    )
    ...
    if mode == "unknown":
        raise SystemExit(
            "❌ Position mode unknown; refusing to place orders. Fix SDK/permissions and retry."
        )
    ...
    if args.smoke_order:
        ...
        LOG.info(
            "Placing tiny STOP_MARKET reduce-only order (symbol=%s side=%s stop=%.4f mode=%s posSide=%s)",
            symbol,
            side,
            stop,
            mode,
        ...
        LOG.info(
            "✅ Smoke order path OK (order_id=%s, client_order_id=%s)",
            resp.get("order_id"),
            resp.get("client_order_id"),
        )
    else:
